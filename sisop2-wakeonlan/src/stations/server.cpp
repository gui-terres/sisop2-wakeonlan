...
void Server::receiveMessages() {
    ...
    serv_addr.sin_port = htons(PORT_TABLE);  // Use a porta apropriada

    // Ligando o socket ao endereço IP e porta
    ...
    while (true) {
        memset(buffer, 0, 256);
        ssize_t n = recvfrom(sockfd, buffer, 255, 0, (struct sockaddr *)&cli_addr, &clilen);
        if (n < 0) {
